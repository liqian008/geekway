...
<%@page import="com.bruce.geekway.model.KlhUserProfile"%>
			...
			<div class="panel panel-default">
				...
				<div class="datatable-media">
					<table class="table table-bordered table-striped">
						...
						<tbody>
							<%
                           	List<KlhUserProfile> userProfileList = (List<KlhUserProfile>)request.getAttribute("userProfileList");
                           	if(userProfileList!=null&&userProfileList.size()>0){
                           		int i=0;
                           		for(KlhUserProfile userProfile: userProfileList){
                           			i++;
                           	%>
							<tr>
								<td><%=i%></td>
								<td><%=userProfile.getUserOpenId()%></td>
								<td><%=userProfile.getNickname()%></td>
								<td><%=userProfile.getRealname()%></td>
								<td><%=userProfile.getMobile()%></td>
								<td><%=userProfile.getCreateTime() != null ? sdf.format(userProfile.getCreateTime()) : ""%></td>
								<td class='text-center'>
									<div class="table-controls">
										<a
											href="./userProfileDisplay?userOpenId=<%=userProfile.getUserOpenId()%>"
											class="btn btn-link btn-icon btn-xs tip" title=""
											data-original-title="资料"><i class="icon-pencil3"></i></a>
										<a
											href="./userScoreLogList?userOpenId=<%=userProfile.getUserOpenId()%>"
											class="btn btn-link btn-icon btn-xs tip" title=""
											data-original-title="积分变更"><i class="icon-pencil3"></i></a>
