...
<body class="sidebar-wide">
	...
	<div class="page-container">
		...
		<div class="page-content">
			...
			<form id="validate" action="<s:url value='./wxWebUserPaging'/>" method="post" >
				<!-- Basic inputs -->
			...
			<div class="panel panel-default">
				...
				<div class="table-responsive">
					<table class="table table-bordered table-striped dataTable">
						...
						<tbody>
							<%
							PagingResult<WxWebUser> pagingResult = (PagingResult<WxWebUser>)request.getAttribute("wxWebUserPagingData");
							List<WxWebUser> wxWebUserList = pagingResult.getPageData();
							if(wxWebUserList!=null&&wxWebUserList.size()>0){
                           		int i=0;
                           		for(WxWebUser wxWebUser: wxWebUserList){
                           			boolean synced = wxWebUser.getSyncStatus()>0;
                           			i++;
                           	%>
							...
							<tr>
		                        ...
		                        <td>
		                        	...
		                        	<img src="<%=wxWebUser.getHeadImgUrl()%>" class="img-media"/>
		                        	</a>
		                        	<%}%>
		                        </td>
		                        <td><%=wxWebUser.getSubscribeStatus()==1?"已关注":"已取消"%></td>
		                        <td><%=synced?wxWebUser.getNickname():"详细信息未同步"%></td>
		                        <td><%=wxWebUser.getOpenId()%></td>
		                        <td>
		                        	<%if(synced){%>
		                        	<%=wxWebUser.getCountry()+"-"+wxWebUser.getProvince()+"-"+wxWebUser.getProvince()%>
		                        	<%}%>
		                        </td>
		                        <td class='text-center'>
		                        	<div class="table-controls">
		                        		<%String link = "./historyMessageDialog?openId="+wxWebUser.getOpenId();%>
											<a href="<%=link%>"
												class="btn btn-link btn-icon btn-xs tip" title=""
												data-original-title="回 复"><i class="icon-bubble3"></i></a>
		                        		<%if(synced){%>
										<a href="./wxWebUserInfo?wxWebUserId=<%=wxWebUser.getId()%>"
											class="btn btn-link btn-icon btn-xs tip" title=""
