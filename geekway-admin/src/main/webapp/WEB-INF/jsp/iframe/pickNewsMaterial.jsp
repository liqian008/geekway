					...
					<table class="table table-bordered table-striped table-check">
						...
						<tbody>
							...
							<tr>
		                        ...
		                        <td>
									<%
										//String meterialLink = ProductUtil.getArticleLink(materialNews.getId());
									%>
									<a href="<%=materialNews.getLink()%>" target="_blank">预览</a>
								</td>
		                        <td>正常</td>
		                        <td class='text-center'>
		                        	<div class="table-controls">
										<a href="javascript:pick(<%=materialNews.getId()%>, '<%=materialNews.getLink()%>')" class="material-picker btn btn-link btn-icon btn-xs tip" title="" data-original-title="选择"><i class="icon-heart2"></i></a>
									</div>
								</td>
                               </tr>
							<%}
...
function pick(materialId, materialRemark){
	...
	<%}else if(operation==2){//系统群发消息
		%>
		if(confirm('确定要群发多图文消息【'+materialRemark+'】吗？')){
			//ajax post
			var broadcastData = {"materialId": materialId};
			$.post("${pageContext.request.contextPath}/geekway/mpBroadcastNews.json", broadcastData, function(responseData) {
    			var result = responseData.result;
   				if(result==1){
   					alert("多图文消息群发成功，正在返回对话群发列表页面...");
   				}else{
   					alert("多图文消息群发失败，正在返回对话群发列表页面...");
   	   			}
   				parent.location.reload();
    		 }, "json");
